...
mkShowInstance mode mb_ctxt ty (DataDecl _ _ cons) = do
  ...
  constraints <- mkShowContext mode <$>
                 inferConstraintsDef mb_ctxt (DConT showName) ty cons
  arg_ty <- mk_arg_ty mode ty
  ...
  where
    ...
    go (DNormalC True tys@[_, _])
      ...
      = do argL   <- newUniqueName "argL"
           argLTy <- newUniqueName "argLTy"
           argR   <- newUniqueName "argR"
           argRTy <- newUniqueName "argRTy"
           ...
           return $ DClause [DVarP p, DConP con_name' [ mk_var_pat mode argL argLTy
                                                      , mk_var_pat mode argR argRTy ]] $
             mk_ShowSing'SigE mode [argLTy, argRTy] $
    ...
    go (DNormalC _ tys) = do
      ...
      let show_args     = map (showsPrecE appPrec1) args
          ...
          named_args = DVarE composeName
                         ...
                         `DAppE` composed_args
      return $ DClause [DVarP p, DConP con_name' $ zipWith (mk_var_pat mode) args argTys] $
        mk_ShowSing'SigE mode argTys $
        DVarE showParenName
...
data ShowMode = ForPromotion     -- ^ For promotion/singling.
              | ForShowSing Name -- ^ For a 'Show' instance for a singleton type.
                                 --   Includes the name of the original data type.
...
-- TODO RGS: Docs
mk_arg_ty :: Quasi q => ShowMode -> DType -> q DType
mk_arg_ty ForPromotion ty = pure ty
mk_arg_ty (ForShowSing ty_tycon) ty = do
  z <- qNewName "z"
  pure $ DConT (singTyConName ty_tycon) `DAppT` (DVarT z `DSigT` ty)

-- TODO RGS: Docs
mk_var_pat :: ShowMode -> Name -> Name -> DPat
mk_var_pat ForPromotion  var_name _       = DVarP var_name
mk_var_pat ForShowSing{} var_name ty_name =
  DVarP var_name `DSigP` (DConT singFamilyName `DAppT` DVarT ty_name)

-- TODO RGS: Docs
mk_ShowSing'SigE :: ShowMode -> [Name] -> DExp -> DExp
mk_ShowSing'SigE ForPromotion  _    e = e
mk_ShowSing'SigE ForShowSing{} args e =
  e `DSigE` DForallT [] (map (\arg -> DConT showSing'Name `DAppT` DVarT arg) args)
                        (DConT showSName)
